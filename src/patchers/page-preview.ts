    ...
    plugin.register(around(pagePreviewInstance, {
        onLinkHover(old) {
            return function (hoverParent: HoverParent, targetEl: HTMLElement | null, linktext: string, sourcePath: string, state: any): void {
                ...
                if ((!sourcePath || sourcePath.endsWith('.pdf')) && plugin.settings.hoverHighlightAction === 'open' && state?.pdfPlusBacklinkOpenParams) {
                    lib.workspace.openBacklinkFromPDF(state.pdfPlusBacklinkOpenParams);
                    return;
                }
